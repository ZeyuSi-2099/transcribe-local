...
- [ ] **脱敏质检「报告说修了、稿子里没修上」。** 2026-09-15 本机用真模型跑后处理时发现：
      脱敏把一处数值模糊化后，与紧邻的原话字面重合，形成相邻重复；质检报告写「发现相邻重复，已删除」，
      交出的脱敏稿里重复仍在（日志 `自检+质检：修正 2 条 → 落地 1 条`，没落地的那条报告里照样记成已修）。
      两个问题：① 质检修正没落地；② 没落地的修正不该在报告里写成已修 —— 报告与稿子对不上，用户会信报告。
      代码在 `backend/pipeline/pp_redact_ds.py` 步 4（自检+质检的修正落地与 `_qc_md` 出报告），与线上同源。
      Duner 2026-09-15 定为远期，**与线上一起改**，本地不先改。
- [ ] **定字把静音块写成「（静音）」一行。** 同一次发现：终稿里出现一行只有「（静音）」的句子。
      代码里没有这句，是定字模型按提示词自己写的；量过那段录音，确实没人说话（比前后说话处低约 9 dB），**没有丢内容**，
      只是多出一行无用的占位，也会被带进后处理的输入。
      可选方向：提示词里说清静音块不出句，或收尾时程序滤掉只含占位的句子（注意别误滤真内容，定不下来的仍标 `[❓]`）。
      `_merge_zh.py` 是生成的、不手改，所以 Duner 2026-09-15 定为远期，**与线上提示词一起改**。
